...
class ACOAlgorithm(private val parameters: ACOParameters = ACOParameters()) {
    ...
    suspend fun run(points: List<Point>, metric: IDistanceMetrics): ACORouteResult {
        ...
        val renderedRoutePoints = if (metric is AStarMetric) {
            buildPolyline(metric, routeWithReturn)
        } else {
            routeWithReturn
        }
    ...
    private fun buildRoutePoints(
        ...
        if (routeIndexes.isEmpty()) return emptyList()
        val route = routeIndexes.map { idx -> points[idx] }
        return if (withReturn && route.size > 1) route + route.first() else route
    }
